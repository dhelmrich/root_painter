...
class RootPainter(QtWidgets.QMainWindow):
    ...
    def mouse_scroll(self, event):
        ...
        if alt_down or shift_down:
            #Â change by 10% (nearest int) or 1 (min)
    ...
    def initUI(self):
        ...
        if os.path.splitext(fname)[1] == '.seg_proj':
            ...
            #Â only warn if -psn not in the args. -psn is in the args when
            #Â user opened app in a normal way by clicking on the Application icon.
    ...
    def update_image(self):
        ...
        self.graphics_view.image = image_pixmap #Â for resize later
        self.im_width = im_width
        self.im_height = im_height

        self.scene.setSceneRect(-15, -15, im_width+30, im_height+30)

        #Â Used to replace the segmentation or annotation when they are not visible.
    ...
    def update_seg(self):
        ...
        if os.path.isfile(self.seg_path):
            ...
            if sys.platform == 'win32':
                #Â For some reason the text has a different size
        ...
        def check():
            ...
            for m in messages:
                ...
                try:
                    ...
                    #Â being used by another process
    ...
    def show_hide_seg(self):
        #Â show or hide the current segmentation.
        if self.seg_visible:
            self.seg_pixmap_holder.setPixmap(self.blank_pixmap)
            self.seg_visible = False
        else:
    ...
    def show_hide_image(self):
        #Â show or hide the current image.
        #Â Could be useful to help inspect the segmentation or annotation
    ...
    def show_hide_annot(self):
        #Â show or hide the current annotations.
        #Â Could be useful to help inspect the background image
    ...
    def open_pre_segment_count_dialog(self):
        ...
        #Â scroll+pan gets switched on here.
